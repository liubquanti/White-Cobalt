...
class _StorageUsageScreenState extends State<StorageUsageScreen> {
  ...
  Map<String, dynamic> _deviceStorage = {'total': 0, 'free': 0, 'used': 0, 'cobaltPercentage': 0.0};
  ...
  Future<void> _loadStorageData() async {
    ...
    try {
      ...
      List<ServiceStorage> data =
          serviceToSizeMap.entries.map((entry) => ServiceStorage(entry.key, entry.value)).toList();
  ...
  Future<void> _loadDeviceStorageInfo() async {
    ...
    try {
      ...
      print(
          'Cobalt is using: ${filesize(_totalSize)} (${_deviceStorage['cobaltPercentage'].toStringAsFixed(2)}%)');
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(LocaleKeys.StorageUsage.tr()),
        centerTitle: true,
        backgroundColor: Colors.black,
        leading: IconButton(
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          LocaleKeys.WhenYouDownloadFilesUsingWhiteCobaltHereWillBeStorageUsageStats
                              .tr(),
                    ...
                    child: Column(
                      ...
                      children: [
                        Column(
                          children: [
                            Text(
                              LocaleKeys.Total.tr(args: [
                                "${filesize(_totalSize)} / ${filesize(_deviceStorage['total'])}"
                              ]),
                              style: const TextStyle(
                                fontSize: 14,
                                fontWeight: FontWeight.bold,
                          ...
                          children: _storageData.asMap().entries.map((entry) {
                            ...
                            return Container(
                              margin: EdgeInsets.only(
                                  bottom: index == _storageData.length - 1 ? 0 : 10),
                              decoration: BoxDecoration(
                                color: const Color(0xFF191919),
                                borderRadius: BorderRadius.circular(11),
                                border: Border.all(
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          backgroundColor: Colors.black,
          title: Text(
            LocaleKeys.DeleteFiles.tr(args: [serviceName]),
          ...
          content: SizedBox(
            ...
            child: Padding(
              ...
              child: Text(
                LocaleKeys.AreYouSureYouWantToDeleteAllDownloadedFilesFrom.tr(args: [serviceName]),
  ...
  Future<void> _deleteDirectory(String serviceName) async {
    try {
      ...
      if (hasDeleted) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(LocaleKeys.FilesDeletedSuccessfully.tr(args: [serviceName])),
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(LocaleKeys.NoDirectoriesFoundForThisService.tr()),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(LocaleKeys.CouldNotDeleteDirectory.tr(args: [e.toString()])),
