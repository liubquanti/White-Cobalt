...
import '../services/pinstances.dart';
...
class _InstancesScreenState extends State<InstancesScreen> {
  ...
  List<PartnerServer> _partnerServers = [];
  String _error = '';
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final partnersFuture = PartnerServersService.fetchPartnerServers();

      final officialServers = await officialFuture;
      final instancesResponse = await instancesFuture;
      final partnerServers = await partnersFuture;
    ...
    return DefaultTabController(
      ...
      child: NestedScrollView(
        headerSliverBuilder: (context, innerBoxIsScrolled) {
          ...
          final bool hasPartners = _partnerServers.isNotEmpty;
            ...
            headerChildren
              ..add(
                Padding(
                  ...
                  child: Text(
                    LocaleKeys.PartnerInstances.tr(),
                    style: const TextStyle(
                      fontSize: 16,
              ...
              ..addAll(
                _partnerServers.map(_buildPartnerServerCard),
              )
  ...
  Widget _buildPartnerServerCard(PartnerServer server) {
    return Card(
      margin: const EdgeInsets.only(bottom: 10),
      color: const Color(0xFF191919),
      shape: RoundedRectangleBorder(
              ...
              children: [
                Material(
                  ...
                  child: InkWell(
                    ...
                    onTap: () async {
                      await Future.delayed(const Duration(milliseconds: 250));
                      await _addPartnerServerWithConfirmation(server);
                    },
                    child: Container(
                      padding: const EdgeInsets.all(6),
                      decoration: BoxDecoration(
  ...
  Future<void> _addPartnerServerWithConfirmation(PartnerServer server) async {
    _apiKeyController.clear();

    final bool requiresAuth = server.auth == true;
    final String? envApiKey = requiresAuth ? _resolveApiKeyForHost(server.api) : null;
    ...
    return showDialog(
      ...
      builder: (dialogContext) {
        final bool showTextField = !requiresAuth || envApiKey == null;
        ...
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              const SizedBox(height: 16),
              if (requiresAuth && envApiKey != null)
                Text(
                  LocaleKeys.PartnerApiKeyAutoApplied.tr(),
                  style: const TextStyle(
                    fontSize: 12,
                    color: Colors.white70,
                  ),
                ...
                Column(
                  ...
                  children: [
                    Text(
                      LocaleKeys.PartnerApiKeyMissing.tr(),
                      style: const TextStyle(
                        fontSize: 12,
                        color: Colors.white70,
                      ),
              ...
              if (showTextField) ...[
                Text(
                  requiresAuth && envApiKey == null
                      ? LocaleKeys.APIKey.tr()
                      : LocaleKeys.APIKeyOptional.tr(),
                  style: const TextStyle(
                    fontSize: 14,
                  ),
          ...
          actions: [
            ...
            TextButton(
              onPressed: () async {
                ...
                final manualKey = _apiKeyController.text.trim();
                final String resolvedApiKey;

                if (requiresAuth) {
                  resolvedApiKey = (envApiKey ?? manualKey).trim();

                  if (resolvedApiKey.isEmpty) {
                    ScaffoldMessenger.of(dialogContext).showSnackBar(
                      SnackBar(
                        content: Text(LocaleKeys.APIKeyRequired.tr()),
                      ),
                    );
                    return;
                  }
                } else {
                  resolvedApiKey = manualKey;
                }

                final serverConfig = ServerConfig(
                  server.apiUrl,
                  resolvedApiKey.isNotEmpty ? resolvedApiKey : null,
                );
  ...
  String? _resolveApiKeyForHost(String host) {
    final String sanitized = host
        .toUpperCase()
        .replaceAll(RegExp(r'[^A-Z0-9]'), '_')
        .replaceAll(RegExp(r'_+'), '_');

    final List<String> candidates = [
      'PARTNER_${sanitized}_API_KEY',
      'API_KEY_${sanitized}',
      '${sanitized}_API_KEY',
      sanitized,
      host,
